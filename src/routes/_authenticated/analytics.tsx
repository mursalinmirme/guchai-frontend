...
import {
  useTasksByDate, useTasksInRange, todayStr, elapsedSeconds, plannedDurationSeconds, type Task,
  ...
  return (
    <div>
      <header className="h-20 border-b border-border flex items-center justify-between px-8 bg-bg-primary/80 backdrop-blur-md sticky top-0 z-20">
        <div>
          <h1 className="text-sm text-text-dim">Analytics</h1>
          <p className="text-xl font-bold tracking-tight">Performance Report</p>
        </div>
          ...
          {(["day", "week", "month", "year"] as RangeKey[]).map((r) => (
            <button
              ...
              className={`px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-widest transition ${
                range === r ? "bg-brand text-brand-foreground" : "text-text-dim hover:text-text-main"
              }`}
            >{r}</button>
          ))}
        </div>
...
function DayView() {
  ...
  return (
    <div className="grid grid-cols-12 gap-6">
      <KpiCard className="col-span-3" label="Total tasks" value={String(tasks.length)} />
      <KpiCard className="col-span-3" label="Completed" value={String(totals.onTime + totals.overtime)} accent="text-success" />
      <KpiCard className="col-span-3" label="Overtime" value={String(totals.overtime)} accent="text-danger" />
      <KpiCard className="col-span-3" label="Skipped" value={String(totals.skipped)} accent="text-text-dim" />

      <ChartCard title="Planned vs Actual (minutes)" className="col-span-8">
        {perTask.length === 0 ? (
      ...
      <ChartCard title="Outcome mix" className="col-span-4">
        {pieData.length === 0 ? (
      ...
      <ChartCard title="Time totals" className="col-span-12">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 p-2">
          ...
          <TotalStat label="Overtime accrued" value={fmt(tasks.reduce((s, t) => {
            const e = elapsedSeconds(t), p = plannedDurationSeconds(t);
            return s + Math.max(0, e - p);
          }, 0))} accent="text-danger" />
          <TotalStat label="Utilization" value={planned ? Math.round((worked / planned) * 100) + "%" : "—"} accent="text-success" />
        </div>
      </ChartCard>
    </div>
  );
...
function WeekView() {
  ...
  const rows = useMemo(() => days.map((d) => {
    ...
    return {
      day: format(d, "EEE"),
      planned: Math.round(dayTasks.reduce((s, t) => s + plannedDurationSeconds(t), 0) / 3600 * 10) / 10,
  ...
  return (
    <div className="grid grid-cols-12 gap-6">
      <ChartCard title="Hours worked vs planned (this week)" className="col-span-8">
        <ResponsiveContainer width="100%" height={320}>
          <BarChart data={rows}>
            <CartesianGrid stroke="rgba(255,255,255,0.05)" vertical={false} />
      ...
      <ChartCard title="Tasks completed" className="col-span-4">
        <ResponsiveContainer width="100%" height={320}>
          <LineChart data={rows}>
            <CartesianGrid stroke="rgba(255,255,255,0.05)" vertical={false} />
            <XAxis dataKey="day" stroke={MUTED} fontSize={11} />
            <YAxis stroke={MUTED} fontSize={11} />
            <Tooltip contentStyle={tooltipStyle} />
            <Line type="monotone" dataKey="completed" stroke={SUCCESS} strokeWidth={2} dot={{ r: 4 }} />
          </LineChart>
        </ResponsiveContainer>
      </ChartCard>
    </div>
...
function YearView() {
  ...
  return (
    <div className="grid grid-cols-12 gap-6">
      <ChartCard title="Hours worked (year)" className="col-span-6">
        <ResponsiveContainer width="100%" height={320}>
          <BarChart data={rows}>
            <CartesianGrid stroke="rgba(255,255,255,0.05)" vertical={false} />
      ...
      <ChartCard title="Tasks completed (year)" className="col-span-6">
        <ResponsiveContainer width="100%" height={320}>
          <LineChart data={rows}>
            <CartesianGrid stroke="rgba(255,255,255,0.05)" vertical={false} />
            <XAxis dataKey="month" stroke={MUTED} fontSize={11} />
...
function KpiCard({ label, value, accent, className }: { label: string; value: string; accent?: string; className?: string }) {
  return (
    <div className={`glass-card rounded-2xl p-5 ${className ?? ""}`}>
      ...
      <p className={`text-3xl font-mono font-bold mt-1 ${accent ?? "text-text-main"}`}>{value}</p>
    </div>
  );
}
