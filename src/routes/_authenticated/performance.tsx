...
function Performance() {
  ...
  const daily = useMemo(() => {
    ...
    return days.map((d) => {
      ...
      return { date: format(d, "d MMM"), hours: Math.round(worked / 3600 * 10) / 10, planned: Math.round(planned / 3600 * 10) / 10, score };
    });
  }, [tasks]);
  ...
  const streak = useMemo(() => {
    ...
    for (let i = daily.length - 1; i >= 0; i--) {
      if (daily[i].hours > 0) s++; else break;
    }
    return s;
  }, [daily]);
  ...
  return (
    <div>
      <header className="h-20 border-b border-border flex items-center justify-between px-8 bg-bg-primary/80 backdrop-blur-md sticky top-0 z-20">
        <div>
          <h1 className="text-sm text-text-dim">Performance</h1>
          <p className="text-xl font-bold tracking-tight">Motivation & momentum</p>
        </div>
      </header>

      <div className="p-8 space-y-8">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
          <Kpi icon={<Flame className="text-danger" />} label="Current streak" value={`${streak}d`} note="Consecutive days worked" />
          <Kpi icon={<Trophy className="text-warning" />} label="7-day score" value={`${Math.round(rollingScore)}%`} note="Rolling completion" />
          <Kpi icon={<Zap className="text-brand" />} label="This week" value={fmt(weekWorked)} note={`${weekTasks.length} tasks scheduled`} />
          <Kpi icon={<Target className="text-success" />} label="This month" value={fmt(monthWorked)} note={`${monthCompleted}/${monthTasks.length} completed`} />
        </div>

        <div className="glass-card rounded-3xl p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-bold flex items-center gap-2"><TrendingUp className="size-4 text-brand" /> Last 30 days — hours worked</h3>
          ...
          <ResponsiveContainer width="100%" height={280}>
            <AreaChart data={daily}>
              ...
              <Area type="monotone" dataKey="hours" stroke="var(--brand)" strokeWidth={2} fill="url(#hoursGrad)" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="glass-card rounded-3xl p-6">
          ...
          <div className="grid grid-cols-15 md:grid-cols-30 gap-1">
            {daily.map((d, i) => {
              const c = d.hours === 0 ? "bg-white/5"
                : d.score >= 80 ? "bg-success"
                : d.score >= 50 ? "bg-brand"
                : d.score >= 20 ? "bg-warning"
                : "bg-danger";
              return (
                <div key={i} title={`${d.date} · ${d.score}% · ${d.hours}h`} className={`aspect-square rounded ${c}`} />
              );
            })}
          </div>
...
function Kpi({ icon, label, value, note }: { icon: React.ReactNode; label: string; value: string; note: string }) {
  return (
    <div className="glass-card rounded-2xl p-5 flex flex-col gap-2">
      ...
      <p className="text-[11px] text-text-dim">{note}</p>
    </div>
  );
}

function Insight({ tone, title, body }: { tone: "brand" | "warning" | "success"; title: string; body: string }) {
  const map = { brand: "border-brand/30 text-brand", warning: "border-warning/30 text-warning", success: "border-success/30 text-success" } as const;
  return (
